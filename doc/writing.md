...
При запуске тестового набора используются два специальных метода: ```setUp``` и ```tearDown```. Первый вызывается перед началом запуска тестов в наборе, второй - после окончания работы последнего теста набора. Тесты определяются аннотациями к методам класса.
